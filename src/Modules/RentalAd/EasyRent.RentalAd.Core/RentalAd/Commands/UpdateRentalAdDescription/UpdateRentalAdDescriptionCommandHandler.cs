...
public record UpdateRentalAdDescriptionCommandHandler : IRequestHandler<UpdateRentalAdDescriptionCommand, UpdateRentalAdDescriptionResponse>
{
    private readonly ILogger _logger;
    private readonly IAggregateRepository _aggregateRepository;

    public UpdateRentalAdDescriptionCommandHandler(ILogger logger,
    ...
    public async Task<UpdateRentalAdDescriptionResponse> Handle(UpdateRentalAdDescriptionCommand request, CancellationToken cancellationToken)
    {
        ...
        rentalAd.UpdateDescription(RentalAdDescription.FromString(request.NewDescription));

        await _aggregateRepository.SaveAsync<Domain.RentalAd.RentalAd, RentalAdId>(rentalAd);

        _logger.Info("Rental Ad with ID: {RentalAdId} description has been updated in event store", rentalAd.Id.Value);

        return new UpdateRentalAdDescriptionResponse();
    }
}