...
public record UpdateRentalAdTitleCommandHandler : IRequestHandler<UpdateRentalAdTitleCommand, UpdateRentalAdTitleResponse>
{
    private readonly ILogger _logger;
    private readonly IAggregateRepository _aggregateRepository;

    public UpdateRentalAdTitleCommandHandler(ILogger logger,
    ...
    public async Task<UpdateRentalAdTitleResponse> Handle(UpdateRentalAdTitleCommand request, CancellationToken cancellationToken)
    {
        ...
        rentalAd.UpdateTitle(RentalAdTitle.FromString(request.NewTitle));

        await _aggregateRepository.SaveAsync<Domain.RentalAd.RentalAd, RentalAdId>(rentalAd);

        _logger.Info("Rental Ad with ID: {RentalAdId} title has been updated in event store", rentalAd.Id.Value);

        return new UpdateRentalAdTitleResponse();
    }
}